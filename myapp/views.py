...
def paymentsuccess(request):
    ...
    notification.send_email(
        subject='Grand Casa Society Monthly Maintenance',
...
def amenitypaymentsuccess(request):
    ...
    notification = NotificationService()
    notification.send_email(
        subject='Grand Casa Society Amenity Booking Confirmation',
...
def admin_delete_notice(request, nid: int):
    ...
    try:
        ...
        # Clear all notice-related cache
        cache.delete_pattern("notices:*")
        if deleted_count == 0:
...
def changeStatus(request,id):
    ...
    c.status='Approved'
    c.save()
    u = User.objects.filter(email=c.uid.email)  # email should go to authenticated user only
    to = u[0].email
    if c.status=='Approved':
        notification = NotificationService()
        notification.send_email(
            subject='Grand Casa Society Refund Amount',
            message="We have refunded "+ str(c.amount) +" amount into your account successfully. Thank you..! ",
            to_email=[to]
        )
